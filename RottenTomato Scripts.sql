...
select count(1),(date_part('decade', release_date) * 10) as decade
from movies
group by decade
...
select count(entertainment_workers.name), entertainment_workers."name" names
from  movies  
...
where movie_workers.credit_type = 'actor'
group by entertainment_workers."name"
order by count desc 
limit 10;
